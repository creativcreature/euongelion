...
describe('reference-volume grounding retrieval', () => {
  it('returns grounding hits for short user input plus scripture reference', async () => {
    const hits = await retrieveReferenceHits({
  ...
  it('returns a deterministic fallback hit even when lexical matches are weak', async () => {
    const hits = await retrieveReferenceHits({
