...
// In-memory lock to prevent concurrent generation for the same plan
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Update the plan day in the store
      // The createPlan already stored placeholder days; we need to update the content.
      // Since the repository uses in-memory store, we update directly.
      const repoModule = await import('@/lib/soul-audit/repository')
      const allStoredDays = repoModule.getAllPlanDays(planToken)
      const targetDayRecord = allStoredDays.find(
        (d) => d.day_number === nextPendingDay.day,
      )
      if (targetDayRecord) {
        targetDayRecord.content = generatedDay
      }
