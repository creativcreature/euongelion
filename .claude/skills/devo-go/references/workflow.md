...
- **Two-Minute Open first (SA-030, forward-only):** every day (teaching days AND recap; sabbath is already its own short form) begins with a self-contained ~2-minute devotional — scripture (anchor, as is) → vocab (the day's word) → reflection (one compact prompt) → prayer (short) → `cta` (`ctaLabel: "DEEP DIVE"`, `ctaHref: "#devotional-section-6"`, subtext inviting the full reading). It must stand alone: stopping at the CTA = a complete devotional. Declare `"format": "two-minute-open"` at the day's top level; the validator BLOCKS if the first five modules aren't exactly this sequence. The deep dive below keeps its own full-length reflection set and prayer — the opening pair is compact, not a substitute; don't duplicate the exact wording between the two.
...
- Teaching modules carry `chiasm_position` + `pardes_layer` (internal only). Reading level scales 8th → 12th → college → 12th → 8th.
