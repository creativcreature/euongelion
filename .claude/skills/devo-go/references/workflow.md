...
- **Image intensity 1–5 (SA-131)** — ask it here, with the rest. 1 = current site register, 5 = cinematic baroque. Stills 1 / motion 5 are DEFAULTS only; a named intensity applies to every generated image in the run and overrides the split (founder, 2026-08-29). It is a series-level setting recorded in the brief header, and every Phase-7 generation carries it as `--intensity=N`.
