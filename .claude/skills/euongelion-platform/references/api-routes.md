...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userInput || userInput.length < 10) {
      return Response.json({ error: 'Please share more about what\'s on your heart.' }, { status: 400 });
    }
    ...
    if (existingToken) {
      ...
      if (existingSession && existingSession.soul_audit_count >= 3) {
        return Response.json({ error: 'Maximum Soul Audits reached. Browse series instead.' }, { status: 400 });
      }
    }
    ...
    const { data: allSeries } = await supabaseAdmin
      .from('series')
      .select('slug, title, subtitle, soul_audit_keywords, emotional_tones, life_circumstances')
      .eq('published', true);
...
export async function GET(req: NextRequest) {
  try {
    ...
    const currentDayNum = Math.min(session.current_day, Math.max(...availableDays, 1));
    const todayContent = series.days.find((d: any) => d.day_number === currentDayNum);
    ...
    return Response.json({
      ...
      today: {
        ...
        unlock_time: lockedDays.length > 0 
          ? getNextUnlockTime(session.start_date, lockedDays[0], session.timezone) 
          : null,
      },
      progress: {
