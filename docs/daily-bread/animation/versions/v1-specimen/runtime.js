// ============================================================================
// Runtime: plates → print shader
const circlePts = (x, y, r, n = 48) => Array.from({ length: n }, (_, i) => [x + Math.cos(i / n * TAU) * r, y + Math.sin(i / n * TAU) * r]);
...
function createScene(canvas){
  ...
  return {
    /** sceneT: position in the loop; printT: time since the page began (drives the one-time print-in and grain boil). */
    render(sceneT, printT, W, H){
      plates.begin(W, H);
      drawFrame(plates, ((sceneT % LOOP) + LOOP) % LOOP);
      ...
      gl.viewport(0, 0, W, H);
      gl.uniform1f(u('u_time'), printT); gl.uniform2f(u('u_resolution'), W, H); gl.uniform1f(u('u_dot'), Math.max(4, W / 320));
...
if (params.has('capture')) {
  ...
  // captures skip the print-in so any frame, and the loop seam, render as they play mid-film
  window.__render = (t) => scene.render(t, t + 100, c.width, c.height);
  window.__loop = LOOP;
  window.__ready = true;
} else {
  ...
  const frame = (now) => {
    ...
    const dt = last ? Math.min(100, now - last) / 1000 : 0; last = now; clock += dt;
    for (const b of bands) {
      if (!b.visible || !b.scene) continue;
      ...
      b.time = reduced.matches ? 2.6 : b.time + dt;
      b.scene.render(b.time, reduced.matches ? 100 : clock, w, h);
    }
  };
  requestAnimationFrame(frame);
}
