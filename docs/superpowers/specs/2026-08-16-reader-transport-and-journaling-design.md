...
**Decisions proposed:** SA-062 (two-state site model), SA-058 (Audible-modeled transport), SA-061 (unified journaling)
...
## 2. SA-062 — The two-state site model
...
**Gate flip is blocked on a verified live sign-in.** SA-062 may be built behind the existing flags, but the signed-out wall does not go live until an end-to-end sign-in has been performed against production and the received email shown to the founder.
...
- `retention-cleanup` purges **anonymous** sessions after 30 days and never touches authenticated rows. Under SA-062 no anonymous annotation rows will exist at all, which simplifies it.
...
| 7   | SA-062 auth-state sweep, locked-not-hidden, anonymous-persistence removal (§2) | 3, 6       |
...
| 14  | "two site states… no account, data should not be retained"                | §2 (SA-062)                                                                                 | Approved, scoped                        |
