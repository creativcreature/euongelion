...
| 2026-06-10 | Elevation v3.0 — generation timeout fix (SA-020/SA-022). Root cause of the live "Day N took too long (25000ms deadline)": the day prompt targeted a 3,000-4,000 word / 5-section chiastic PaRDeS essay at `maxOutputTokens: 6000`, impossible inside the 25s wall-clock deadline. Right-sized to the brief's 600-900 word bespoke edition (same JSON schema/keys; tight section targets; `maxOutputTokens: 2800`). Unblocks the flagship and cuts per-day cost ~8×. Still on the configured model (provider choice deferred per founder). Grounding/RAG/validation untouched; no determinism added. |
