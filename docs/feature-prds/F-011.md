...
| 2026-02-15 | pending | SA-013                 | F-011      | Realigned shell IA to canonical nav (`HOME`, `SOUL AUDIT`, `DAILY BREAD`, `SERIES`), removed masthead subline, added centered pronunciation line, added desktop/mobile auth entry points, and completed footer/help/legal route wiring to remove dead-end navigation states.                                                                                                                      | `type-check`, `lint`, contracts, tracking, feature-prd checks        | 8.7 -> 9.0           | Physical-device UX polish + founder acceptance still pending.             |
| 2026-02-15 | pending | SA-013, SA-014         | F-011      | Removed duplicated mobile `SETTINGS` path from secondary nav to keep a single authenticated account entry (`ACCOUNT`), and added authenticated-mobile regression coverage to prevent duplicate menu state from returning.                                                                                                                                                                         | `lint`, `type-check`, `test`, `verify:*`                             | 9.0 -> 9.2           | Route walkthrough and physical-device QA evidence still pending.          |
| 2026-02-15 | pending | SA-013, SA-014         | F-011      | Strengthened global scroll recovery by removing stale Lenis lock classes (`lenis`, `lenis-smooth`, `lenis-scrolling`, `lenis-stopped`) from both `html` and `body` on route lifecycle events, plus added contract tests for unlock invariants.                                                                                                                                                    | `lint`, `type-check`, `test`, `verify:*`                             | 9.2 -> 9.3           | Physical-device scroll-stress QA still required (iOS + Android).          |
| 2026-02-18 | pending | SA-013, SA-014         | F-011      | Added shell-level stale lock cleanup in `EuangelionShellHeader` so route/menu transitions clear residual lock styles (`overflow`, `position`, touch/overscroll props), lock classes, and lock data attributes on both `html`/`body`; added regression coverage for mount + route-change cleanup.                                                                                                  | `lint`, `type-check`, `test`, `verify:*`                             | 9.3 -> 9.4           | Physical-device scroll-stress QA still required (iOS + Android).          |
| 2026-02-20 | pending | SA-010                 | F-011      | Fixed mobile nav correctness drift by promoting `SERIES` into primary mobile navigation, deduping primary/secondary item partitioning via set-based filtering, and moving primary links into a horizontal scroll-safe lane to avoid wrap collisions that made menu behavior feel broken on small viewports. Also added nav isolation to reduce sticky overlap interference.                       | `lint`, `type-check`, `test` (`shell-header`, sticky contracts)      | 9.4 -> 9.6           | Physical-device route walkthrough still needed for final founder signoff. |
| 2026-06-10 | pending | SA-010, SA-013         | F-011      | Audit fix: the mobile menu "ACCOUNT" link (shown to signed-in users) pointed to a non-existent `/account` route — a hard 404 on every tap. Repointed to `/settings` (the account home, consistent with the desktop account menu's Library/Settings/Help). Reconciled stale nav/masthead test assertions (`shell-header`, `smoke`) to the current kicker copy "The Good News, for You. Every Day." | `lint`, `type-check`, `test` (`shell-header`, `smoke`), `verify:*`   | 9.6 -> 9.7           | Physical-device route walkthrough still pending.                          |
| 2026-06-10 | pending | SA-010, SA-013         | F-011      | UX-audit tranche C (menu/wayfinding): the reader "LIBRARY" menu had 6 links but 5 (`/daily-bread?tab=today                                                                                                                                                                                                                                                                                        | archive                                                              | notes                | highlights                                                                | chat-history`) silently landed on the default view — `/daily-bread`never reads`?tab=`. Reduced to three links that resolve to real, distinct surfaces (Today's Reading→`/daily-bread`, Your Library→`/library`, Bookmarks→`/saved`). Footer label "My Library"→"Library" to match the nav + account menu. Auth: sign-in error copy now names the likely cause (expired/used link) + recovery; added an honest one-line value statement under "Welcome back" (account just syncs across devices; reading works without one). | `lint`, `type-check`, `test` (1195), `verify:*` | 9.7 -> 9.8 | Larger IA consolidation (4 "my stuff" surfaces; canonical reader URL) is a founder decision, tracked separately. |

| 2026-06-10 | pending | SA-010, SA-013 | F-011 | UX-audit tranche D (mobile/desktop parity + onboarding jargon): added a `Help` link to the mobile hamburger menu (it lived only in the desktop account dropdown + footer — unreachable for the mobile-first audience). Removed the raw LLM-vendor picker ("Default Brain": OpenAI/Google/MiniMax/NVIDIA Kimi) + Open-Web checkbox from first-run onboarding (implementation jargon vs. sacred-minimalism + "AI as composer"); still configurable in Settings. | `lint`, `type-check`, `test` (1195), `verify:*` | 9.8 -> 9.9 | Homepage CTA hierarchy, scripture-first reorder, canonical reader URL folded into the v3.0 elevation brief. |
