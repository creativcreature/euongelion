...
| 2026-02-20 | pending | SA-003, SA-004 | F-020      | Removed site-cookie consent as a blocker for devotional-path selection so Soul Audit option clicks always proceed to consent/select API calls and route immediately. Cookie notice now controls optional analytics preference only, aligning consent UX with non-blocking essential functionality best practices.                        | `type-check`, `eslint`, targeted tests             | 8.5 -> 9.0  | Capture live Safari/mobile evidence for first-visit consent + selection continuity.                        |
