...
| 2026-02-20 | pending | SA-013, SA-014 | F-021      | Hardened split resilience when curated matches are concentrated in a single series: keeps 3 AI options anchored to curated candidates and fills prefab slots from series metadata fallback instead of failing the whole submit path. Preserved fail-closed behavior when curated candidates are completely unavailable. | `full test suite`, `verify:*`                                         | 9.1 -> 9.3           | Manual QA still needed for founder review of fallback copy/preview quality. |
