...
| 2026-05-04 | pending | OVERNIGHT-2026-05-04 / SA-013, SA-018 | F-022      | Active-plan visibility wired beyond `/` and `/daily-bread`: new `useActivePlan` hook + `ActivePlanBadge` (header + tile variants), mounted in `EuangelionShellHeader` (desktop topbar + mobile secondary nav) and at the top of `/series`. `/api/soul-audit/current` now returns `seriesTitle` and `dayNumber` so the badge label renders without bundling `SERIES_DATA` for the lookup.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `type-check`, `lint`, focused vitest (5 new + 7 regression pass) | +0.0        | Plan-management UI in `/settings` and a `/library` consolidation surface remain P1 for a later session. |
| 2026-06-10 | pending | SA-013, SA-014                        | F-022      | UX-audit fix (launch-blocker): the new-plan selection handoff was broken — `results/page.tsx` did `router.push(payload.route)`, but the async select contract returns `{jobId, status:'pending', pollUrl}` with no `route`, so first-time plan selection pushed `undefined` (spinner stopped, no navigation, no feedback during the 30–90s build). Wired the previously-dead `GenerationProgress` to poll the job and navigate on completion; retry now re-submits a fresh job; `SoulAuditSelectResponse` is now honest about the async branch. Honesty/copy: "BUILDING YOUR PLAN"→"FINDING YOUR PATHS"; approved gentle validation copy; removed slot-machine "reroll / X-of-1 left" and metered "Audit X of N" framing; de-jargoned ("Reload Options"→"Refresh options", "Monthly clean house"→"Clear old paths"); point-of-entry privacy line + `aria-live` status + textarea `maxLength`; prompt pills no longer overwrite typed text. | `lint`, `type-check`, `test`, `build`, Workers `preview`         | 8.4 -> 8.8  | E2E generation walk-through in Workers preview before relying on it.                                    |

| 2026-06-10 | pending | SA-013, SA-014 | F-022 | Elevation v3.0 — the "hot off the press" generation loader (founder asked for this repeatedly). Replaced the breathing-dot `GenerationProgress` with an on-brand newspaper press-room: masthead + crimson rule + "SETTING YOUR EDITION", an animated galley proof setting type line-by-line with an ink sweep, "A seven-day edition, being set for you", the REAL server progress ("Composing day N of 7…") driving a progress bar, and reassurance ("about a minute — nothing here is recycled"). Embraces the ~30-90s wait instead of racing a deadline. Resilience: a slow/errored unit transparently re-kicks (auto-retry ×2) behind the loader so the reader never sees "took too long"; submit options call also retries transparently. Reduced-motion safe. | `lint`, `type-check`, Workers `preview` + screenshot | 8.8 -> 9.0 | Edition still won't COMPLETE on Sonnet (every day errors at the 25s deadline) — provider switch required (next). |
