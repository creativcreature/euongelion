...
- M00: Unified methodology (backend optimization — no UX change)

## Acceptance Criteria

### Desktop

- Soul Audit plan generation completes with ≤6 LLM calls per plan (down from 9)
- Feature flags toggle between deterministic and LLM paths without errors
- Outline cache returns cached results for repeated theme+tone combinations
- No regression in devotional content quality for Days 1-5

### Mobile

- Same as desktop — all token-saving optimizations are server-side and device-agnostic
- No change to client-side Soul Audit UX or loading behavior
