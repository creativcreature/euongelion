...
| 2026-06-09 | pending | SA-018                 | F-035      | **Amended SA-018 (founder-authorized).** Bookmark save/remove (`/api/bookmarks` POST/DELETE) now allowed anonymously — keyed by the audit session token (`getOrCreateAuditSessionToken`), with the signed-in `user.id` taking precedence and merging on sign-in. This reverses the bookmark portion of the 2026-02-18 gate so the anonymous reading flow's Save button works. Annotations (`/api/annotations`) **remain sign-in-gated**. Added `/saved` route + `SavedList` view; dead `/daily-bread?tab=bookmarks` nav link repointed to `/saved`. Synced `save-state-auth-gate.test.ts`, `production-decisions.yaml` (SA-018 title + note), and `PRODUCTION-SOURCE-OF-TRUTH.md` #2. | `lint`, `type-check`, `test`, `verify:*`, `build`, Workers `preview` + curl | 7.5 -> 7.5           | Annotations intentionally still gated; a fully anonymous note/highlight model is a separate decision. |
