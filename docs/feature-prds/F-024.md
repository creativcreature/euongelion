...
| 2026-02-21 | pending | SA-045         | F-024      | Added a first-audit guest gate in results flow: unauthenticated users are prompted to sign up before devotional entry, with a soft-onboard fallback (Sabbath, theme, text size) when continuing as guest. Also hydrated `planDays` on existing selection responses and added resilient plan-day fallback resolution so missing numbered days (including day 2 gaps) no longer disappear from retrieval flows. | `type-check`, `lint`                     | 9.7 -> 9.8  | Add dedicated regression coverage for guest gate handoff and historical plan day-number gap recovery.                |
