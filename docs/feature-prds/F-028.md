...
| 2026-06-13 | pending | SA-020         | F-028      | Finish-line Wave 2 length/depth gate: checkDepth() runs after verify() and fails the gate (re-roll, no canned fallback) when a day is thin - reading >=600 words / >=2 paragraphs / verbatim anchor blockquote / real prayer; deep-dive >=1800 words / >=3 sections. Shortfalls surfaced in telemetry + job error, distinct from citation failures. | type-check, lint 0, grounded-endnotes-depth tests (12) | 4 -> 7      | Tune thresholds against live generations.                                     |
