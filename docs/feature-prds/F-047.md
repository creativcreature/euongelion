...
| 2026-06-13 | pending | SA-013         | F-047      | Checkout HARD-DISABLED (founder: not launch-ready): new BILLING_CHECKOUT_LIVE master switch (default off) — checkout route 503s on all initiation regardless of Stripe/IAP config; config route mirrors it so Settings offers no Subscribe CTA; disabled copy -> 'coming soon'. Verified live: no functional checkout path. | type-check, lint 0, full suite 1434 pass, live config paymentsEnabled=false | 7 -> 7      | Flip BILLING_CHECKOUT_LIVE=true only when billing is launch-ready.                             |
