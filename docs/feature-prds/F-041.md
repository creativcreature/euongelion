...
| 2026-02-18 | pending | SA-013, SA-014 | F-041      | Hardened scroll reliability by preventing Lenis stopped-state overflow locking from forcing persistent no-scroll behavior (`.lenis.lenis-stopped` now resolves to auto overflow).                                                                                                                                                                                                                          | `lint`, `type-check`, `test`, `verify:*`                                                                                                                   | 6.9 -> 7.2  | Physical-device stress QA still required for long-scroll pages on mobile. |
| 2026-06-13 | pending | SA-013         | F-041      | Finish-line Wave 2 performance: global containment guards (media max-width, `overflow-wrap:anywhere` on foreign-script/reference/URL surfaces, `min-width:0` flex shrink guards on scroll rails, `overflow-x:auto` on markdown pre/tables); CLS reservation for the substack banner (aspect-ratio); content-visibility LCP fix preserved; newspaper shell NOT clipped (sticky-safe). Covers F-041/042/043. | type-check, lint 0, Playwright 375-1440 both themes = 0 doc overflow (incl. injected 120-char token), layout-overflow + lcp-cls contract tests, 1419 suite | 4 -> 7      | Lighthouse mobile field re-measure on prod.                               |
