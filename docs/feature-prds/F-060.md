...
- Consent folded into select route (`essentialAccepted` in request body) — delete separate consent route
...
- SA-035: Select route uses `buildDeterministicDay()` directly — zero LLM, zero timeout risk
- SA-035: `composeDay()` catches LLM throws internally so fallback always fires
- SA-035: No empty-reflection days possible — every path produces real reference content
- SA-036: Metadata chunks (document headers, TOCs, PG boilerplate) filtered from index
- SA-036: 9 new public domain sources added: Chesterton, Pascal, Bunyan, Owen, Bounds, Hannah Whitall Smith
- SA-036: Index expanded to 5,271 chunks across 19 sources (was 2,404 from 8 sources)
- SA-037: Day 1 LLM-composed (flowing prose), Days 2-5 deterministic (instant). Dead code removed.
- OVERNIGHT-2026-05-04 / SA-005, SA-020: AI-plan reader adapter `aiPlanDayToReader` (`src/lib/soul-audit/ai-plan-to-reader.ts`) maps composer-emitted flat fields (`scriptureText`, `reflection`, `prayer`) into `DevotionalModule[]` so `PlanDayContent.tsx` and `DayContent.tsx` dispatch through `ModuleRenderer` instead of falling back to three bare `<p>` tags. `nextStep` and `journalPrompt` continue to render in the existing bottom section to keep behavior consistent with the curated reader (deviation noted in `docs/overnight-followups.md`). 10 unit tests pass; type-check + lint clean.

## Methodology References

- M00 (canonical IA / navigation UX) — Soul Audit pipeline overhaul (retro-registered 2026-07-10)

## Device Contracts

### Desktop

- Pipeline overhaul; shipped May 2026. Behavior identical across breakpoints (server-side pipeline work; no bespoke desktop surface).
