...
BRAND_BOILERPLATE = (
    "POSTERIZED TRITONE SCREEN-PRINT POSTER — bold flat-shape illustration with halftone dot fill, "
    "Roy Lichtenstein meets Constructivist propaganda meets vintage Penguin paperback cover. "
    "EXACTLY this look: take a recognizable classical scene and reduce every form to bold flat "
    "color shapes with NO photographic detail — every figure is a stack of solid posterized "
    "shape-blocks (one shape per tonal step). Mid-tones rendered as halftone dot stipple between "
    "the flat color planes. Strong hard cast shadows as flat geometric shape blocks under figures. "
    "Limited TRITONE palette — only three colors total in the entire image: "
    "65% Newspaper Cream paper #F0ECE6 ground (heavy halftone dots in shadow zones), "
    "25% Cobalt #1F2A8D for primary figure-shapes / sky / architecture, "
    "10% Burgundy #8E3F3F for accent figure-shapes / robes / wood / blood. "
    "(Other valid tritones: Cream + Cobalt + Olive #6F8F4F | Cream + Cobalt + Sacred Gold #C19A6B | "
    "Cream + Burgundy + Sacred Gold | Cream + Navy Ink #11182A + Cobalt). "
    "DENSE halftone dot pattern visible in every gradient region (Lichtenstein-style benday dots). "
    "Bold geometric flat-shape rendering throughout — anatomy reduced to silhouette-blocks, faces "
    "as flat-shape forms with minimal feature detail (no realistic skin texture, no rendered eyes), "
    "garments as solid color planes, architecture as bold geometric blocks. "
    "Anonymous classical poses — figures recognizable by silhouette and gesture, NOT by individual "
    "face. Faces are flat color planes with abstract minimal features. NO HOODS, NO COWLED ROBES, "
    "NO MONK-CLOAKS — period-appropriate plain tunics rendered as flat shapes. "
    "Bold sharp graphic shadow planes cast on cream paper ground, halftone dot density at edges. "
    "Heavy paper grain texture across the entire field. Slight ink misregistration at color "
    "boundaries (each color slightly off-register from the others, as in real screenprint). "
    "References: Roy Lichtenstein 1960s benday-dot pop-print, Saul Bass film-poster reduction, "
    "Atelier Populaire May '68 tritone silkscreen, Constructivist propaganda flat-shape, "
    "vintage Penguin Modern Classics covers. "
    "NO photorealism, NO continuous-tone shading, NO smooth gradients (only halftone-dot gradients), "
    "NO modern 3D-render look, NO cartoon Disney register, NO saccharine tract-illustration. "
